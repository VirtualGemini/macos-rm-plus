...
**What to build:** Let users run `tc` in dry-run mode to preview the complete top-level Trash Plan without changing the filesystem. Basic path inputs become domain-level Trash Inputs, retain input order, report their kind, and are checked against protected-path policy before a plan is presented.
...
- [x] Filesystem root, the current working directory, the user's home directory, and their equivalent path expressions are rejected as Protected Paths with exit code 1.
