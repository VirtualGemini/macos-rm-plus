...
integration boundary. Recoverability of `tc` deletions is degraded relative
...
Deleting a file with the production `tc` produces a Trash item with a
working Finder "Put Back" entry on the first deletion. After restoring the
item with Put Back and deleting the same file again with `tc`, the new Trash
item shows no Put Back entry about 90% of the time (reporting environment,
macOS 26.5.1). The item itself reaches the Trash on every run and `tc` exits
...
- A Finder delete → Put Back → delete cycle keeps the Put Back entry at any
  ...
  `tc` usually loses it, so `tc` is observably worse than Finder for
...
1. `tc <file>` — the Trash item shows Put Back.
2. Restore it with Finder Put Back.
3. Within a few seconds, run `tc <file>` again for the same name.
...
Does the second `tc` Trash Operation omit Put Back metadata, or does another
process remove or replace metadata that `tc` successfully caused the system
Trash API to write?

### Environment and safety boundary

- Reporting environment: macOS 26.5.1, home Trash, Finder Trash window open,
  human-driven Finder Put Back, then same-name re-trash through `tc`.
...
- [Swift `PutBackMetadataScanner`](../../../TestSupport/TrashTestKit/PutBackMetadataScanner.swift)
  rewrites the former Python scanner as pure read-only test support.
- [Swift command-line probe](../../../TestSupport/TrashTestKit/PutBackMetadataProbe.swift)
  restores the former scanner's file-reading and text-output entrypoint without
  adding a Trash capability.
- [Swift scanner tests](../../../Tests/TrashPlatformTests/PutBackMetadataScannerTests.swift)
...
- The former independent `trash.swift` was not copied as a second direct
  ...
  product path with the built `tc` executable and keeps the independent-caller
...
```sh
xcrun swiftc -warnings-as-errors -D TC_PUT_BACK_METADATA_PROBE \
  TestSupport/TrashTestKit/PutBackMetadataScanner.swift \
  TestSupport/TrashTestKit/PutBackMetadataProbe.swift \
  -o /tmp/tc-put-back-metadata-probe

/tmp/tc-put-back-metadata-probe \
  "/Volumes/<scratch>/.Trashes/$(id -u)/.DS_Store"
```

No executable disposable-volume Trash runner is checked in. The current test
safety contract requires real test Trash Operations to use `tc-test` inside
`~/tc-test/test/<run-uuid>/` and explicitly rejects mount points and
...
| --- | --- | --- |
| `tc-put-back-race-12` | First production `tc` Trash Operation on `/source/` | Immediately showed `ptbL=/source/` and `ptbN=tc-put-back-race-12`. |
...
| `-12-d` | Finder `trash`-container restore, 1 second wait, production `tc` re-trash | Fresh `ptbL`/`ptbN` records were still present at 2 and 4 seconds. |
...
| --- | --- | --- | --- | --- |
| E1 | Does `tc` write Put Back metadata on every call? | Run six same-name trash/restore cycles on the scratch volume and scan `.DS_Store` after each Trash Operation. Repeat from a different original directory. | Every Trash Operation produced one current `ptbL`/`ptbN` record; changing the source directory changed the stored original location. | The first and repeated `tc` paths do not differ at metadata-write time. |
...
| E4 | Is the conflict caused by `tc` code outside the system API call? | Replace `tc` with the archived minimal Swift caller, which invokes only `FileManager.trashItem`. Run four cycles with a 1.5-second delay. | One of four cycles lost the fresh Put Back record completely. | The failure reproduces for an independent caller of the same Foundation API and is not specific to `tc` parsing, planning, or execution code. |
...
This is a product defect at the macOS Trash integration boundary: `tc` cannot
...
The metadata loss was observed seconds after the `tc` process exited. The
...
1. Trash the current probe file through the selected production `tc` binary.
...
matched. No `tc` error, timeout, hang, or candidate Automation prompt occurred.
...
the Workspace candidate. Do not overwrite an installed `tc` between rounds.
...
when a shell alias or PATH shim named `rm` invokes `tc`. Renaming or wrapping the
...
- Production and `tc-test` reach the capability only through the existing
...
- A reset denial round returned `finder_automation_denied (not_moved)` for
  `/tmp/tc-finder-automation.kMh4lh/automation-deny-probe`; the source remained
...
compile-time-isolated Swift process under `tc-test`, in two restore variants:
...
`-5000`. Production `tc` is unaffected: it only asks Finder to `delete`, which
...
performed the actual Finder "Put Back" command; `tc-test` detected the restore
...
Interpretation. The matched control for this scenario is E6: production `tc`
...
expired while Finder waited on a human. For a user this means `tc <symlink>`
...
`tc`, and requiring every user to grant their terminal Full Disk Access is a
far larger privilege than `tc` needs.
...
`/Volumes/tc-pb-c04fc7ca/.Trashes/501/`, Finder initially returned `-1728` before the documented
...
single-process Swift `tc-test put-back-race` scenario. The 30-cycle metadata and
...
children with exact `.tc-finalizer-<canonical-lowercase-uuid>` names. Pure tests pass; the repeated
...
enter production `tc`, and accept only their exact expected warning. The full pure suite passes
...
cleanup, and whitelist checks unchanged while replacing only the hidden `.tc-finalizer-<uuid>`
...
This falsifies hidden helper naming as the cause; changing `.tc-finalizer-<uuid>` to a visible
