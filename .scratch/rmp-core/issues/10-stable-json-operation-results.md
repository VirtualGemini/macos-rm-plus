...
- [ ] Stdout contains exactly one complete JSON document with no human-readable success line in JSON mode; warnings and diagnostics do not corrupt it.
- [ ] JSON-mode tests prove that an accepted `-P` still warns on stderr with TTY or non-TTY input while stdout remains exactly one complete JSON document and the warning alone does not change a successful exit code.
