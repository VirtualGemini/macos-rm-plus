...
- Align public CLI exit-status values with macOS `rm` while retaining rmp's Trash behavior: success
  and moved warnings return `0`, operational and safety failures return `1`, and usage failures
  return `64`; `-W` remains explicitly unsupported.
- Ratchet the production line-coverage baseline from 97.31% to 97.39% with Exit Status Compatibility
