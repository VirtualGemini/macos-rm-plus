...
struct PlanningErrorRenderer {
  ...
  func render(_ error: TrashPlanningError) -> CommandResult {
    ...
    case .noInputs:
      message =
        "tc: \(error.code.rawValue): --dry-run requires at least one Trash Input\n"
    case let .missingPath(path):
      message =
        "tc: \(error.code.rawValue): Trash Input does not exist: "
        + "\(renderer.renderPath(path))\n"
    case let .inaccessiblePath(path):
      message =
        "tc: \(error.code.rawValue): Trash Input cannot be inspected: "
        + "\(renderer.renderPath(path))\n"
    case let .protectedPath(path, protectedPath):
      message =
        "tc: \(error.code.rawValue) (\(protectedPath.rawValue)): "
        + "Protected Path rejected: \(renderer.renderPath(path))\n"
    case let .unavailableProtectedPath(path, protectedPath):
      message =
        "tc: \(error.code.rawValue) for "
        + "\(renderer.renderPath(path)): "
