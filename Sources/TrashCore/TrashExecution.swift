...
struct SingleTrashExecutor<FileSystem: TrashPlanningFileSystem> {
  ...
  private func failureExplanation(code: TrashErrorCode, sourceUnchanged: Bool) -> String {
    ...
    case .finderAutomationConsentRequired:
      return
        "Finder Automation permission is required. Run tc from an interactive desktop session "
        + "and allow the invoking terminal or tc to control Finder. \(sourceState)"
    case .finderAutomationDenied:
      return
        "Finder Automation permission was denied. Enable the invoking terminal or tc in System "
