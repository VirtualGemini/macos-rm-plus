...
struct ManualPutBackWaiter {
  ...
  func putBack(
    ...
    guard
      ...
      name.hasPrefix("tc-test-\(context.runID.uuidString.lowercased())-")
    else {
  ...
  private static func instruction(
    ...
    let nextStep =
      ...
      ?? "tc-test re-trashes it the moment it returns; waiting up to \(Int(timeout))s."
    return """
  ...
  private static func watchRunDirectory(context: TestSafetyContext) throws -> DirectoryChangeWatch {
    ...
    let source = DispatchSource.makeFileSystemObjectSource(
      ...
      queue: DispatchQueue(label: "tc-test.put-back-race.run-directory")
    )
