...
enum PutBackRaceAcceptance {
  ...
  static func runManual(
    ...
    let waiter = ManualPutBackWaiter(
      ...
      postRestoreInstruction: retrashWithProductionFinalizer
        ? "After Put Back is observed, tc-test immediately trashes the production symbolic-link "
  ...
  private static func makeFixture(
    ...
    let body = Data("tc Put Back race \(context.runID.uuidString.lowercased()) \(suffix)\n".utf8)
    switch kind {
    ...
    case .brokenSymbolicLink:
      return try context.createFixtureSymbolicLink(
        suffix: suffix,
        target: "tc-test-\(context.runID.uuidString.lowercased())-absent-target"
      )
    }
  }
