...
#if !TC_TESTING
  #error("tc-test must only be built with TC_TESTING enabled")
#endif
...
private enum TCTestEntrypoint {
  static func execute(arguments: [String]) -> Never {
    ...
    if arguments.first == "--version" {
      FileHandle.standardOutput.write(Data("tc-test build=TC_TESTING\n".utf8))
      exit(0)
    }
    ...
    let result = TestSafetyDriver.runWithInjectedRuntime(arguments: arguments) {
      ...
      let message =
        "tc-test build=TC_TESTING run=\(context.runID.uuidString.lowercased()) ready\n"
...
private func executeOrderedBatch(arguments: [String]) -> Never {
  let result = TestSafetyDriver.runWithInjectedRuntime(
    ...
    try TCTestEntrypoint.currentRuntime()
  } operation: { context, paths in
    guard paths.isEmpty else {
      throw TestSafetyDiagnostic(
        code: .invalidCommandArguments,
...
TCTestEntrypoint.execute(arguments: Array(CommandLine.arguments.dropFirst()))
