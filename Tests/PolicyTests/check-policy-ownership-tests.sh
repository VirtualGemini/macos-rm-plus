...
printf '%s\n' 'gate*' >"$TEMP_DIR/repo/.policy-files"
printf '%s\n' 'gate* @maintainer' 'gate.sh @outsider' >"$TEMP_DIR/repo/.github/CODEOWNERS"
if "$TEMP_DIR/repo/scripts/check-policy-ownership.sh" >/dev/null 2>&1; then
  echo "test failure: later outsider CODEOWNERS rule was ignored" >&2
  exit 1
fi
