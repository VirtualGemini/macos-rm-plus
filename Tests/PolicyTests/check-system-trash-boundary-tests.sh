...
TEMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/tc-system-trash-boundary-tests.XXXXXX")
...
mkdir -p \
  "$repo/scripts" \
  "$repo/Sources" \
  "$repo/Sources/TrashPlatform" \
  "$repo/Sources/tc" \
  "$repo/TestSupport/TrashTestSafety" \
  "$repo/Tests/TrashPlatformTests" \
  "$repo/TestSupport/tc-test" \
  "$repo/FutureTarget"
cp "$ROOT/scripts/check-system-trash-boundary.sh" "$repo/scripts/"

cat >"$repo/Sources/TrashPlatform/FinderTrashClient.swift" <<'EOF'
let script = NSAppleScript(source: "tell application id \"com.apple.finder\"")
let event = NSAppleEventDescriptor.list()
EOF
cat >"$repo/TestSupport/TrashTestSafety/FoundationSymlinkTrashClient.swift" <<'EOF'
let client = FileManager.default
try client.trashItem(at: target, resultingItemURL: &result)
EOF
cat >"$repo/Sources/tc/main.swift" <<'EOF'
let client = MacOSTrashClient()
EOF
cat >"$repo/Sources/TrashPlatform/MacOSTrashClient.swift" <<'EOF'
let finder = FinderTrashClient()
try FileManager.default.trashItem(at: target, resultingItemURL: &result)
EOF
cat >"$repo/TestSupport/TrashTestSafety/WhitelistedTrashClient.swift" <<'EOF'
let client = FinderTrashClient()
let symlinkClient = FoundationSymlinkTrashClient()
EOF
cat >"$repo/TestSupport/TrashTestSafety/WhitelistedPutBackClient.swift" <<'EOF'
let script = NSAppleScript(source: "tell application id \"com.apple.finder\"")
let event = NSAppleEventDescriptor.list()
EOF
cat >"$repo/TestSupport/TrashTestSafety/PutBackRaceAcceptance.swift" <<'EOF'
let client = WhitelistedPutBackClient(context: context)
EOF
cat >"$repo/Tests/TrashPlatformTests/FinderTrashClientTests.swift" <<'EOF'
let client = makeInjectedFinderTrashClient(finderDelete: spy.call)
EOF
cat >"$repo/Tests/TrashPlatformTests/MacOSTrashClientTests.swift" <<'EOF'
let client = makeInjectedMacOSTrashClient(
  finderTrash: finder.call,
  foundationTrash: foundation.call
)
EOF
cat >"$repo/TestSupport/TrashTestSafety/WhitelistedMacOSTrashClient.swift" <<'EOF'
let client = makeInjectedMacOSTrashClient(
  finderTrash: finder.call,
  foundationTrash: foundation.call
)
EOF
cat >"$repo/Tests/TrashPlatformTests/FoundationSymlinkTrashClientTests.swift" <<'EOF'
let client = FoundationSymlinkTrashClient(systemTrash: spy.call)
EOF
cat >"$repo/Tests/TrashPlatformTests/WhitelistedTrashClientTests.swift" <<'EOF'
...
EOF
cat >"$repo/Tests/TrashPlatformTests/FoundationTrashFinalizerTests.swift" <<'EOF'
let client = WhitelistedTrashClient.testingOnly(
  context: context,
  authorization: authorization,
  systemTrash: spy.call
...
EOF
cat >"$repo/Tests/TrashPlatformTests/WhitelistedMacOSTrashClientTests.swift" <<'EOF'
let client = WhitelistedTrashClient.testingOnly(
  context: context,
  authorization: authorization,
  systemTrash: spy.call
...
EOF
cat >"$repo/Tests/TrashPlatformTests/PutBackRaceAcceptanceTests.swift" <<'EOF'
...
EOF
cat >"$repo/TestSupport/tc-test/main.swift" <<'EOF'
print("safe")
EOF
