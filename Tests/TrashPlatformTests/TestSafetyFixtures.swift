...
final class SafetyHomeFixture {
  ...
  init() throws {
    homeURL = FileManager.default.temporaryDirectory.appendingPathComponent(
      "tc-test-safety-\(UUID().uuidString)",
      isDirectory: true
    )
  ...
  var containerURL: URL { homeURL.appendingPathComponent("tc-test", isDirectory: true) }
  var authorizedRootURL: URL { containerURL.appendingPathComponent("test", isDirectory: true) }
  var containerMarkerURL: URL { containerURL.appendingPathComponent(".tc-test-container") }
  var rootMarkerURL: URL { authorizedRootURL.appendingPathComponent(".tc-test-root") }
  ...
  func runDriver() -> TestSafetyDriverResult {
    TestSafetyDriver.run(
      ...
      runtime: .testing(executableName: "tc-test", trustedUser: trustedUser),
