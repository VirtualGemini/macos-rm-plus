...
@Suite("TrashPlatform scaffold", .serialized)
struct TrashPlatformScaffoldTests {
  @Test("TrashPlatform and TrashTestKit targets are available")
  func platformTargetsAreAvailable() {
    #expect(TrashPlatformModule.name == "TrashPlatform")
    #expect(TrashTestKitModule.name == "TrashTestKit")
  }
  ...
  func foundationPlanningAdapterClassifiesSymbolicLinks() throws {
    let rootURL = FileManager.default.temporaryDirectory.appendingPathComponent(
      "tc-planning-links-\(UUID().uuidString)",
      isDirectory: true
    )
  ...
  func foundationPlanningAdapterClassifiesSpecialAndMissingEntries() throws {
    let rootURL = FileManager.default.temporaryDirectory.appendingPathComponent(
      "tc-planning-special-\(UUID().uuidString)",
      isDirectory: true
    )
    defer { try? FileManager.default.removeItem(at: rootURL) }
    try FileManager.default.createDirectory(at: rootURL, withIntermediateDirectories: false)
