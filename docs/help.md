...
All paths are planned before confirmation, and approved inputs are passed to the macOS Workspace
Trash API serially in input order. Each success reports the exact destination from the system's
source-to-destination mapping. Failure never triggers permanent deletion, a
`FileManager.trashItem` fallback, direct Trash-directory access, overwrite, or automatic move-back.
...
deletion in Finder. The Workspace-backed candidate remains subject to the issue 12 maintainer-run
Finder differential before release.
