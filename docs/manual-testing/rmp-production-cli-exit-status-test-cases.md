# rmp 生产式 CLI 退出码无人值守测试

本文档基于
[`rmp-production-cli-test-cases.md`](rmp-production-cli-test-cases.md)
重新整理，只验证当前生产式 `rmp` 的退出码契约。测试不要求人工输入、不打开 Finder、不请求
...
- 默认使用当前仓库构建 Release 二进制，再安装到独立临时目录后执行。也可以通过绝对路径
  `RMP_BINARY=/path/to/rmp` 指定已经构建且与待测提交一致的 Release 二进制。
