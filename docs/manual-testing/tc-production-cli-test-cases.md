...
本文件用于真实 macOS 主机测试。命令均使用安装后的 `tc`，不使用 `.build/...` 路径。

每个用例的“状态”和“预期”描述当前分支行为。带日期的“反馈”是当时版本的历史记录；行为切片
完成后，旧反馈可能不再代表当前预期，只有重新执行并记录的新反馈才能替代它。

含确认提示的命令必须单独逐行执行，不得把整个代码块一次性粘贴到终端。否则排在 `tc` 后面的
Shell 文本可能被标准输入读取为确认回答，制造无效响应或错误结果。

## 注意事项：环境污染与「放回原处」

真机测试依赖系统废纸篓与 Finder。「放回原处」失败**不一定**是 `tc` 产品缺陷；常见原因是测试环境污染。`tc` 不维护 Finder 的私有放回 metadata。Workspace 候选已在自动差分中失败；当前候选通过
结构化 Apple Event 让 Finder 执行移动，并会在首次真实操作时请求 Automation 权限。旧的
`FileManager.trashItem` 基线存在已确认的同名快速 restore/re-trash 竞态，见
[issue 12](../../.scratch/macos-trash-cli/issues/12-put-back-metadata-race.md)。不得把该 ticket 的精确复现
重新归类为环境污染。
...
| 情况 | 更宜归类 |
|------|----------|
| 原目录已不存在，放回失败并点名旧 `tmp.xxx` | **环境问题**，不算 `tc` 回归 |
...
# 3) 删除本机残留的 tc 隔离临时目录（仅限确认的测试 footprint，勿扫删无关 tmp）
# 4) 重启 Finder
killall Finder
...
install -m 755 .build/release/tc "$HOME/.local/bin/tc"
...
预期：`command -v tc` 指向 `$HOME/.local/bin/tc`；版本命令退出码为 0。

反馈：

```text
日期: 2026-07-16
分支: test/tc-production-cli
预清理: 用户确认清空废纸篓；终端复核废纸篓计数=0
环境备注: TC-02 误测对象已由用户放回；终端验证旧原路径和内容正确；新一轮使用全新隔离目录
make build-release: 成功
command -v tc: /Users/virtualgemini/.local/bin/tc
tc --version: tc 0.1.0（exit=0）
...
预期：等价于 `tc -f file-after`；退出码 0；`-f` 不被当作文件名。
...
stderr: tc: warning: -P does not securely overwrite; the item will only be moved to Trash
exit=0
source=absent（移动后）
...
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stdout: 空
stderr: tc: missing_input: Trash Input does not exist: "missing-file"
exit=1
废纸篓计数=0（未调用 Trash）
结果: PASS
```
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
root-exit=3 stderr: tc: protected_path (filesystem-root): Protected Path rejected: "/"
home-exit=3 stderr: tc: protected_path (home-directory): Protected Path rejected: "/Users/virtualgemini"
cwd-exit=3 stderr: tc: protected_path (current-directory): Protected Path rejected: ".../tmp.tbyfgQFr3V"
stdout 均为空；废纸篓计数=0
结果: PASS
```
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: protected_path (filesystem-root): Protected Path rejected: "/"
exit=3
stdout: 空；废纸篓计数=0
结果: PASS
```
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: unknown option "--not-an-option"
exit=2
...
输入: n
stderr 结果: tc: confirmation_declined for "interactive-file": confirmation was declined; the Trash Input was not moved
exit=1
...
预期退出码：64。stderr 为 `tc: at least one Trash Input is required`；不检查文件系统，不调用 Trash。
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: at least one Trash Input is required
exit=2
...
# 出现逐项确认提示后输入 y 并按回车
tc --interactive file-interactive-long
printf 'exit=%s\n' "$?"
...
tc --confirm=smart file-confirm-smart
printf 'exit=%s\n' "$?"
...
# 出现批量确认提示后输入 y 并按回车
tc --confirm=smart directory-confirm-smart
printf 'exit=%s\n' "$?"
...
tc --confirm=never directory-confirm-never
printf 'exit=%s\n' "$?"
...
# 出现批量确认提示后输入 y 并按回车
tc --confirm=once file-confirm-once
printf 'exit=%s\n' "$?"
...
# 出现逐项确认提示后输入 y 并按回车
tc --confirm=each file-confirm-each
printf 'exit=%s\n' "$?"
...
tc --confirm=sometimes file-invalid-confirm
printf 'exit=%s\n' "$?"
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: invalid confirmation mode "sometimes"
exit=2
source=present
结果: PASS
```
...
```sh
tc --confirm= file-invalid-confirm
printf 'exit=%s\n' "$?"
```
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: invalid confirmation mode ""
exit=2
...
```sh
tc --confirm=conditionalOnce file-invalid-confirm
printf 'exit=%s\n' "$?"
```
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: invalid confirmation mode "conditionalOnce"
exit=2
结果: PASS
```
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: unknown option "--confirm"
exit=2
...
```sh
tc --ignore-missing missing-explicit
printf 'exit=%s\n' "$?"
```
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: missing_input: Trash Input does not exist: "missing-fi"
exit=1
...
```sh
tc --force --interactive missing-force-interactive
printf 'exit=%s\n' "$?"
```
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: missing_input: Trash Input does not exist: "missing-force-interactive"
exit=1
结果: PASS
```
...
```sh
tc --interactive --force missing-interactive-force
printf 'exit=%s\n' "$?"
```
...
```sh
tc --ignore-missing -i missing-explicit-i
printf 'exit=%s\n' "$?"
```
...
```sh
tc -f --ignore-missing -i missing-force-explicit-i
printf 'exit=%s\n' "$?"
```
...
```sh
tc --ignore-missing -f -i missing-explicit-force-i
printf 'exit=%s\n' "$?"
```
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: missing_input: Trash Input does not exist: "missing-explicit-force-i"
exit=1
...
tc -i --confirm=never file-i-never
printf 'exit=%s\n' "$?"
...
# 出现逐项确认提示后输入 y 并按回车
tc --confirm=never -i file-never-i
printf 'exit=%s\n' "$?"
...
tc --quiet file-quiet > quiet-success.stdout 2> quiet-success.stderr
exit_code=$?
printf 'exit=%s\n' "$exit_code"
...
```sh
tc --quiet missing-quiet > quiet-error.stdout 2> quiet-error.stderr
status=$?
printf 'exit=%s\n' "$status"
...
exit=1
stdout 字节=0
stderr: tc: missing_input: Trash Input does not exist: "missing-quiet"
结果: PASS
```
...
tc --quiet --verbose file-quiet-verbose
printf 'exit=%s\n' "$?"
```
...
tc --verbose --quiet file-verbose-quiet > verbose-quiet.stdout 2> verbose-quiet.stderr
status=$?
printf 'exit=%s\n' "$status"
...
tc --json --verbose file-json-verbose
printf 'exit=%s\n' "$?"
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: unsupported_output_mode for "file-json-verbose": JSON Trash Operation results are not available in this build
exit=2
source=present
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: conflicting options --json and --quiet
exit=2
source=present
结果: PASS
```
...
tc --quiet --dry-run file-quiet-dry
printf 'exit=%s\n' "$?"
...
tc --non-interactive file-non-interactive
printf 'exit=%s\n' "$?"
...
tc --non-interactive directory-non-interactive
printf 'exit=%s\n' "$?"
...
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.D7KjLFwhRm
确认提示: 无；命令立即返回
stderr: tc: confirmation_required for "directory-non-interactive": confirmation is required before these Trash Inputs can be moved
exit=1
source=present
...
tc --stop-on-error file-stop-single
printf 'exit=%s\n' "$?"
...
# 出现批量确认提示后输入 y 并按回车
tc --stop-on-error file-stop-a file-stop-b
printf 'exit=%s\n' "$?"
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: unsupported_input_count for "file-stop-a", "file-stop-b": single-item execution requires exactly one Trash Input
exit=2
...
tc --strict-options --force file-strict-native
printf 'exit=%s\n' "$?"
...
环境结论: Finder 使用了 FXRecentFolders 中残留的 tmp.tbyfgQFr3V 放回元数据；归类为环境污染，非 tc 移动失败
...
tc -PP file-PP 2> repeated-P.stderr
status=$?
printf 'exit=%s\n' "$status"
...
预期退出码：64。stderr 为 `tc: unsupported Compatibility Option -W`；文件仍在原处。
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: unsupported Compatibility Option -W
exit=2
source=present
结果: PASS
```
...
tc --strict-options -r file-strict-r
printf 'exit=%s\n' "$?"
```
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: Compatibility Option -r is not allowed with --strict-options
exit=2
source=present
结果: PASS
```
...
tc --strict-options -d file-strict-d
printf 'exit=%s\n' "$?"
```
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: Compatibility Option -d is not allowed with --strict-options
exit=2
source=present
结果: PASS
```
...
tc --strict-options -x file-strict-x
printf 'exit=%s\n' "$?"
```
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: Compatibility Option -x is not allowed with --strict-options
exit=2
source=present
结果: PASS
```
...
tc --strict-options -P file-strict-P
printf 'exit=%s\n' "$?"
```
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: Compatibility Option -P is not allowed with --strict-options
exit=2
...
tc --strict-options -W file-strict-W
printf 'exit=%s\n' "$?"
```
...
预期退出码：0。stdout 包含 `Usage: tc [OPTIONS] <PATH>...`、`Native options:` 和 `tc --help -a`；stderr 为空。
...
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
exit=0
stdout 含 Usage: tc [OPTIONS] <PATH>...、Native options:、tc --help -a
stderr: 空
结果: PASS
```
...
预期退出码：0。stdout 包含 `用法：tc [选项] <路径>...`、`原生选项：` 和 `tc --help -a -zh`；stderr 为空。
...
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
exit=0
stdout 含 用法：tc [选项] <路径>...、原生选项：、tc --help -a -zh
stderr: 空
结果: PASS
```
...
预期退出码：64。stderr 为 `tc: -a is only valid with --help`；不检查文件系统。
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: -a is only valid with --help
exit=2
结果: PASS
```
...
预期退出码：64。stderr 为 `tc: -zh is only valid with --help`；不检查文件系统。
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: -zh is only valid with --help
exit=2
结果: PASS
```
...
预期退出码：64。stderr 为 `tc: --help and --version cannot be used together`；stdout 为空。
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: --help and --version cannot be used together
exit=2
stdout: 空
结果: PASS
```
...
预期退出码：0。stdout 固定为 `tc 0.1.0`；stderr 为空，不检查路径或 Trash 能力。
...
tc file-middle-a -f file-middle-b
printf 'exit=%s\n' "$?"
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: unsupported_input_count for "file-middle-a", "file-middle-b": single-item execution requires exactly one Trash Input
exit=2
...
# 出现批量确认提示后输入 y 并按回车
tc file-boundary-a -- file-boundary-b
printf 'exit=%s\n' "$?"
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: unsupported_input_count for "file-boundary-a", "file-boundary-b": single-item execution requires exactly one Trash Input
exit=2
...
tc --dry-run dry-order-first dry-order-directory dry-order-third
printf 'exit=%s\n' "$?"
```
...
tc --dry-run 'file with space' '文件-测试'
printf 'exit=%s\n' "$?"
```
...
tc --dry-run "$newline_name" "$quote_name" "$backslash_name"
printf 'exit=%s\n' "$?"
```
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: unsupported_input_kind (rejected) for "fifo-real": The Trash Input has an unsupported entry kind.
exit=1
source=present
结果: PASS
```
...
tc --dry-run inaccessible-parent/file
status=$?
printf 'exit=%s\n' "$status"
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: inaccessible_input: Trash Input cannot be inspected: "inaccessible-parent/file"
exit=1
恢复权限后 file=present
结果: PASS
```
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: protected_path (current-directory): Protected Path rejected: "."
exit=3
结果: PASS
```
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: protected_path (home-directory): Protected Path rejected: "/Users/virtualgemini/"
exit=3
结果: PASS
```
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: protected_path (parent-directory): Protected Path rejected: ".."
exit=3
结果: PASS
```
...
sudo "$HOME/.local/bin/tc" -f "$PWD/root-safe-file"
printf 'exit=%s\n' "$?"
...
stderr: tc: root_execution: refusing to move Trash Input "/var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.hUTlkLMaoh/root-safe-file" while running as root because Trash ownership and recovery would be unsafe
exit=3
source=present
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: missing_input: Trash Input does not exist: ""
exit=1
结果: PASS
```
...
tc --dry-run --stop-on-error dry-stop-a dry-stop-b
printf 'exit=%s\n' "$?"
```
...
tc --verbose --json file-verbose-json
printf 'exit=%s\n' "$?"
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: unsupported_output_mode for "file-verbose-json": JSON Trash Operation results are not available in this build
exit=2
source=present
...
tc -r --strict-options file-strict-last-r
printf 'exit=%s\n' "$?"
...
tc -d --strict-options file-strict-last-d
printf 'exit=%s\n' "$?"
...
tc -x --strict-options file-strict-last-x
printf 'exit=%s\n' "$?"
...
tc -P --strict-options file-strict-last-P
printf 'exit=%s\n' "$?"
...
tc -W --strict-options file-strict-last-W
printf 'exit=%s\n' "$?"
...
```sh
tc --version no-such-version-path
printf 'exit=%s\n' "$?"
```

预期退出码：0。stdout 固定为 `tc 0.1.0`；不存在的附加路径不会触发 `missing_input`，也不调用 Trash。
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: protected_path (current-directory): Protected Path rejected: ".../tmp.tbyfgQFr3V/."
exit=3
结果: PASS
```
...
# 出现批量确认提示后输入 y 并按回车
tc plain-batch-a plain-batch-b
printf 'exit=%s\n' "$?"
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: unsupported_input_count for "plain-batch-a", "plain-batch-b": single-item execution requires exactly one Trash Input
exit=2
sources=present
...
预期退出码：64。stderr 为 `tc: at least one Trash Input is required`；不构造计划，不调用 Trash。
...
sudo "$HOME/.local/bin/tc" --dry-run "$PWD/root-dry-file"
printf 'exit=%s\n' "$?"
...
```sh
CLANG_MODULE_CACHE_PATH=/tmp/tc-clang-module-cache \
SWIFTPM_MODULECACHE_OVERRIDE=/tmp/tc-swiftpm-module-cache \
...
预期退出码：64。`--` 只结束选项解析，不是路径；stderr 为 `tc: at least one Trash Input is required`。
...
预期退出码：0。stdout 固定为 `tc 0.1.0`；stderr 包含一次 `-P does not securely overwrite` 警告。
...
```sh
tc --dry-run --ignore-missing missing-dry-only
printf 'exit=%s\n' "$?"
```
...
tc --dry-run --ignore-missing dry-mixed-missing dry-mixed-present
printf 'exit=%s\n' "$?"
...
tc -P --dry-run file-P-dry > P-dry.stdout 2> P-dry.stderr
status=$?
printf 'exit=%s\n' "$status"
cat P-dry.stdout
cat P-dry.stderr
...
tc --json json-batch-a json-batch-b
printf 'exit=%s\n' "$?"
...
日期: 2026-07-14
TEST_DIR: /var/folders/l2/09xgvwr91sv001yj_ydqr6sh0000gn/T/tmp.tbyfgQFr3V
stderr: tc: unsupported_input_count for "json-batch-a", "json-batch-b": single-item execution requires exactly one Trash Input
exit=2
